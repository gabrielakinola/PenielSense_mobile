...
import { AlertTriangle, Check, RotateCcw, UserRound } from 'lucide-react-native';
...
export default function IncidentScreen() {
  ...
  const [bodyMarks, setBodyMarks] = useState<Array<{ view: 'FRONT' | 'BACK'; x: number; y: number; type: string }>>([]);
  ...
  return (
    <>
      ...
      <ScreenContainer keyboardShouldPersistTaps="handled">
        ...
        {type === 'FALL' || type === 'INJURY' ? <View style={{ marginBottom: 16 }}><Text style={{ ...typography.label, color: colors.secondary }}>Body map (optional)</Text><Text style={{ ...typography.caption, color: colors.secondary, marginTop: 3 }}>Tap the body to mark each observed injury. This records location only; describe it above.</Text><View style={{ flexDirection: 'row', gap: 8, marginTop: 9 }}>{(['FRONT','BACK'] as const).map((view) => <Pressable key={view} onPress={() => setBodyView(view)} style={{ paddingHorizontal: 12, paddingVertical: 7, borderRadius: radius.full, backgroundColor: bodyView === view ? colors.primary : colors.surfaceElevated }}><Text style={{ ...typography.label, color: bodyView === view ? '#FFF' : colors.secondary }}>{view === 'FRONT' ? 'Front' : 'Back'}</Text></Pressable>)}</View><Pressable onPress={(event) => { const { locationX, locationY } = event.nativeEvent; setBodyMarks((current) => [...current, { view: bodyView, x: Math.round(locationX / 2.2), y: Math.round(locationY / 3.2), type: 'Observed injury' }]); }} style={{ width: 220, height: 320, alignSelf: 'center', marginTop: 10, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}><UserRound size={210} strokeWidth={0.8} color={colors.secondary} />{bodyMarks.filter((mark) => mark.view === bodyView).map((mark,index) => <View key={`${mark.x}-${mark.y}-${index}`} pointerEvents="none" style={{ position: 'absolute', left: `${mark.x}%`, top: `${mark.y}%`, width: 14, height: 14, borderRadius: 7, marginLeft: -7, marginTop: -7, backgroundColor: colors.status.critical, borderWidth: 2, borderColor: '#FFF' }} />)}</Pressable>{bodyMarks.length ? <Pressable onPress={() => setBodyMarks([])} style={{ alignSelf: 'center', flexDirection: 'row', gap: 6, alignItems: 'center', marginTop: 9 }}><RotateCcw size={15} color={colors.secondary}/><Text style={{ ...typography.label, color: colors.secondary }}>Clear {bodyMarks.length} mark{bodyMarks.length === 1 ? '' : 's'}</Text></Pressable> : null}</View> : null}
