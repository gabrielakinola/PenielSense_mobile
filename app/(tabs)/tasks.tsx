...
} from "@/src/services/care-tasks.api";
...
export default function TasksScreen() {
  ...
  const mutation = useMutation({
    ...
    onError: (error) =>
      Alert.alert("Could not record care", normalizeApiError(error)),
  });
  ...
  const chooseOutcome = (task: CareTaskDto) =>
    Alert.alert("Record care outcome", task.title, [
      {
        text: "Completed",
        onPress: () => mutation.mutate({ task, status: "COMPLETED" }),
      },
      {
        text: "Partly completed",
        onPress: () => mutation.mutate({ task, status: "PARTIAL" }),
      },
      {
        text: "Declined",
        onPress: () => mutation.mutate({ task, status: "DECLINED" }),
      },
      {
        text: "Unable / escalate",
        onPress: () => mutation.mutate({ task, status: "ESCALATED" }),
      },
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              description={
                tasksQuery.isError
                  ? normalizeApiError(tasksQuery.error)
                  : "Scheduled care will appear here."
              }
            />
          }
          renderItem={({ item }) => {
